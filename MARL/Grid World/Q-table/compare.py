'''
Compare the results of fully centralized and fully decentralized structure
'''
...
# plot the reward
plt.plot([i for i in range(len(moving_avg))],moving_avg,color='green',label='fully centralized')
plt.legend()
...
if start_q_table1 is None:
    q_table1 = {}
    for p1 in POS_NUM:
        q_table1[p1] = np.random.uniform(-10,0,(5,))
else:
...
if start_q_table2 is None:
    q_table2 = {}
    for p2 in POS_NUM:
        q_table2[p2] = np.random.uniform(-10,0,(5,))
else:
...
if start_q_table3 is None:
    q_table3 = {}
    for p3 in POS_NUM:
        q_table3[p3] = np.random.uniform(-10,0,(5,))
else:
...
moving_avg = np.convolve(episode_rewards, np.ones((SHOW_EVERY,))/SHOW_EVERY, mode="valid")
# plot the reward
plt.plot([i for i in range(len(moving_avg))],moving_avg,color='red',label='fully decentralized')
plt.legend()
